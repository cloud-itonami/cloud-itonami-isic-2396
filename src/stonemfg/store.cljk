(ns stonemfg.store
  "SSoT for the dimension-stone (granite/marble/limestone/sandstone
  slab, monument, countertop) plant-operations coordination actor,
  ...
  (`cloud-itonami-isic-2393`'s own `porcelainmfg.store`, itself
  modeled on `cloud-itonami-isic-2391`'s `refractorymfg.store`), this
  build ships a single `MemStore` backend only (atom of EDN) -- the
  deterministic default for dev/tests/demo, no deps. Per
  docs/adr/0001-architecture.md Decision 1, this vertical is
  self-contained (no external dimension-stone-fabrication capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch (a
                             sawn/polished lot of dimension stone)'s
                             product-type/weight/slab-thickness-
                             tolerance/polish-gloss-level record.
    ...
    - `equipment`         -- a sawing-line/polishing-line unit's own
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled sawing-line/polishing-line-
                             equipment maintenance-window DRAFT
    ...
    - `shipments`         -- a proposed outbound dimension-stone
                             (slab/monument/countertop) shipment DRAFT
                             (`stonemfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (stonemfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (stonemfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :granite-slab
                :material "Absolute Black Granite Slab"
                :weight-kg 20000.0 :slab-thickness-tolerance-mm 1.5 :polish-gloss-level 92.0
                :verified? true :registered? true
                :shipped-weight-kg 4000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :marble-slab
                :material "Carrara White Marble Slab"
                :weight-kg 6000.0 :slab-thickness-tolerance-mm -0.8 :polish-gloss-level 85.0
                :verified? true :registered? true
                :shipped-weight-kg 5600.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :monument
                :material "Rosa Beta Granite Monument Blanks"
                :weight-kg 50000.0 :slab-thickness-tolerance-mm 0.5 :polish-gloss-level 78.0
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"saw-001" {:id "saw-001" :kind :bridge-saw
              :verified? true :registered? true
              :last-maintenance-date "2026-05-01"}
   "polish-002" {:id "polish-002" :kind :polishing-line
                 :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `stonemfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `stonemfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered bridge-saw unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  polishing-line unit (blocks any maintenance scheduling against
