(ns stonemfg.governor
  "Dimension Stone Plant Operations Governor -- the independent
  compliance layer that earns the StoneAdvisor the right to commit.
  ...
  draft-schedule) the sawing line or polishing line, whether a
  shipment proposal's own claimed weight would blow through the
  batch's own logged production weight, or when an act stops being a
  coordination proposal and becomes direct sawing-line/polishing-
  line-equipment control, so this MUST be a separate system able to
  *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is `:dimension-stone-plant-operations-governor`
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:polishing-
                                       line/actuate` or `:sawing-
                                       line/run`) is the 'direct
                                       sawing-line/polishing-line-
                                       equipment control' scope
                                       violation this actor must NEVER
                                       perform -- HARD, PERMANENT,
                                       unconditional.
    4. Sawing/polishing-line-
       actuate blocked             -- for `:schedule-maintenance`, does
                                       the proposal's own `:value`
                                       declare `:actuate-sawing-
                                       polishing-line? true`? Directly
                                       actuating the sawing line or
                                       polishing line is this actor's
    ...
    9. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       through.
   10. Invalid slab-thickness-
       tolerance reading            -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:slab-thickness-tolerance-mm`
                                       that is not a physically
                                       plausible reading
                                       (`stonemfg.registry/
                                       slab-thickness-tolerance-valid?`),
                                       the batch record is rejected
                                       rather than let fabricated/
                                       gauge-error data through.
   11. Invalid polish-gloss-level
       reading                      -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:polish-gloss-level` that is
                                       not a physically plausible
                                       reading
                                       (`stonemfg.registry/polish-
                                       gloss-level-valid?`), the batch
                                       record is rejected rather than
   ...
   12. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [stonemfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct sawing-line/
  polishing-line-equipment-control effect."
...
(defn- sawing-polishing-line-control-blocked-violations
  ...
  Anything else (direct sawing-line/polishing-line control, a
  fabricated actuation effect) is this actor's central scope
  boundary."
  [proposal]
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :sawing-polishing-line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は切断設備・研磨ライン等の直接操作に該当する可能性があり、恒久的に禁止")}]))

(defn- sawing-polishing-line-actuate-blocked-violations
  "HARD, PERMANENT, unconditional: a `:schedule-maintenance` proposal
  whose own `:value` declares `:actuate-sawing-polishing-line? true`
  is attempting to directly actuate the sawing line or polishing line
  -- this actor may only ever propose/schedule a DRAFT maintenance
  window, never actuate the sawing line/polishing line directly. No
  override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-sawing-polishing-line? (:value proposal))))
    [{:rule :sawing-polishing-line-actuate-blocked
      :detail "切断ライン(ガングソー・ブリッジソー・ワイヤーソー等)・研磨ライン(ポリッシュヘッド等)の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- invalid-slab-thickness-tolerance-violations
  "For `:log-production-batch`, if the patch declares a
  `:slab-thickness-tolerance-mm` that is not a physically plausible
  reading, reject rather than let fabricated/gauge-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [stt (:slab-thickness-tolerance-mm (:value proposal))]
      (when (and (some? stt) (not (registry/slab-thickness-tolerance-valid? stt)))
        [{:rule :invalid-slab-thickness-tolerance
          :detail (str stt "mm は物理的に妥当なスラブ厚み公差の範囲外")}]))))

(defn- invalid-polish-gloss-level-violations
  "For `:log-production-batch`, if the patch declares a
  `:polish-gloss-level` that is not a physically plausible reading,
  reject rather than let fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [pgl (:polish-gloss-level (:value proposal))]
      (when (and (some? pgl) (not (registry/polish-gloss-level-valid? pgl)))
        [{:rule :invalid-polish-gloss-level
          :detail (str pgl "GU は物理的に妥当な研磨光沢度の範囲外")}]))))

(defn check
  "Censors a StoneAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (sawing-polishing-line-control-blocked-violations proposal)
                           (sawing-polishing-line-actuate-blocked-violations request proposal)
                           ...
                           (invalid-slab-thickness-tolerance-violations request proposal)
                           (invalid-polish-gloss-level-violations request proposal)))
