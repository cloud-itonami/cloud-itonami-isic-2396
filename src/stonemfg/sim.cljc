(ns stonemfg.sim
  "Demo driver -- `clojure -M:dev:run`. Walks a clean dimension-stone
  ...
  production weight, a proposal that tries to ACTUATE the sawing line
  or polishing line directly (permanently blocked, no override), a
  ...
  an implausible slab-thickness-tolerance reading, and a
  production-batch patch with an implausible polish-gloss-level
  reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on saw-001 (verified, registered, bridge saw -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on saw-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on polish-002 (UNVERIFIED/unregistered polishing line -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on saw-001 with :actuate-sawing-polishing-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible slab-thickness-tolerance reading -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:slab-thickness-tolerance-mm 250.0}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible polish-gloss-level reading -> HARD hold ==")
