(ns stonemfg.advisor
  "StoneAdvisor -- the *contained intelligence node* for the
  dimension-stone (granite/marble/limestone/sandstone slab, monument,
  countertop) plant-operations coordination actor.

  It normalizes production-batch patches (product-type/weight/slab-
  thickness-tolerance/polish-gloss-level), drafts a sawing-line/
  polishing-line-equipment maintenance scheduling proposal against a
  piece of equipment, drafts a safety-concern flag, and drafts an
  outbound dimension-stone shipment coordination proposal against a
  ...
  a committed record and NEVER a real sawing-line/polishing-line
  actuation or freight dispatch. Every output is censored downstream by
  `stonemfg.governor` before anything touches the SSoT -- see README
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct sawing-
                                 ; line/polishing-line-control effect
  ...
  `stonemfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  "Production-batch (sawn/polished lot) intake upsert -- the advisor
  only normalizes/validates the patch; it does not invent the batch's
  product-type, weight, slab-thickness-tolerance, polish-gloss-level
  or verification status. High confidence, low stakes --
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-sawing-polishing-line?=" (boolean (:actuate-sawing-polishing-line? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-sawing-polishing-line? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a sawing-line/polishing-line-equipment safety concern --
  crystalline-silica-dust exposure (respirable silica at sawing/
  polishing), saw-blade/wire-saw kickback or pinch-point hazard, or
  heavy-slab handling/crush hazard. ALWAYS `:stake
  ...
  `stonemfg.phase`: no phase ever adds this op to a phase's `:auto`
  set; `stonemfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound dimension-stone (slab/monument/countertop)
  shipment coordination proposal against a production batch. The
  advisor passes through the caller's own claimed weight -- it does
  NOT invent one, and `stonemfg.governor` NEVER trusts it: it
...
(def ^:private system-prompt
  (str "あなたは石材(御影石・大理石・石灰岩・砂岩のスラブ・墓石・"
       "カウンタートップ)工場プラント運用コーディネーターの助言者です。"
       ...
       "切断設備(ガングソー・ブリッジソー・ワイヤーソー等)・研磨ライン等の設備の"
       "直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `stonemfg.governor`
