(ns stonemfg.phase
  "Phase 0->3 staged rollout for the dimension-stone (granite/marble/
  ...
  consequence (a sawing-line/polishing-line-equipment maintenance
  window means production downtime, and the equipment ends up actually
  touched); it is always a human plant supervisor's call.
  `stonemfg.governor`'s `sawing-polishing-line-actuate-blocked-
  violations` HARD-blocks actuate attempts unconditionally, and the
  confidence/high-stakes gate independently never lets
...
(defn verdict->disposition
  "Map a Dimension Stone Plant Operations Governor verdict to a base
