(ns stonemfg.operation
  "StoneOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (StoneAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the
  Dimension Stone Plant Operations Governor (:govern) and the
...
(defn build
  "Compiles a StoneOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `stonemfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; StoneAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Dimension Stone Plant Operations Governor -- independent censor.
      (g/add-node :govern
