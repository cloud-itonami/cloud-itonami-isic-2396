(ns stonemfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `stonemfg.store` ns docstring for why a second (Datomic-backed)
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (true? (:verified? (store/equipment-unit s "saw-001"))))
    (is (true? (:registered? (store/equipment-unit s "saw-001"))))
    (is (false? (:verified? (store/equipment-unit s "polish-002"))))
    (is (false? (:registered? (store/equipment-unit s "polish-002"))))
    (is (= ["polish-002" "saw-001"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest fresh-store-has-no-batches-or-equipment
  (let [s (store/mem-store)]
    ...
    (is (nil? (store/equipment-unit s "saw-001")))))

(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-type :marble-slab}})
    (is (= :marble-slab (:product-type (store/batch s "batch-001"))))
...
(deftest maintenance-schedule-commits-and-advances-sequence
  (testing "commit-record! (like every sibling actor's own MemStore) returns the store `s`, not the domain result -- inspect the store directly, matching the discipline the actor's own :commit node relies on"
    (let [s (seeded)]
      ...
      (is (= "saw-001" (:equipment-id (store/maintenance s "mnt-1"))))
