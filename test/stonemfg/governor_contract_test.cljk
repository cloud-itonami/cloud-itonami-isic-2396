(ns stonemfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the sawing line or polishing line
  directly... does NOT authorize or execute maintenance/sawing-line-
  polishing-line operations') implemented faithfully. The single
  invariant under test:

    StoneAdvisor never schedules maintenance, flags a safety concern,
    or coordinates a shipment the Dimension Stone Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :marble-slab}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :marble-slab (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-sawing-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest sawing-polishing-line-actuate-is-held-and-permanently-blocked
  (testing "a proposal that sets :actuate-sawing-polishing-line? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
      ...
      (is (not= :interrupted (:status res)))
      (is (some #{:sawing-polishing-line-actuate-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
  ...
  (testing "scheduling the SAME maintenance record twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (exec-op actor "t9a" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                                  :value {:equipment-id "saw-001" :maintenance-type :blade-inspection
                                          :scheduled-date "2026-08-01" :actuate-sawing-polishing-line? false}} coordinator)
          _ (approve! actor "t9a")
...
(deftest invalid-product-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-type :unobtainium-stone}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium-stone (:product-type (store/batch db "batch-001"))) "fabricated product-type never lands in the SSoT")))

(deftest invalid-slab-thickness-tolerance-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:slab-thickness-tolerance-mm 250.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-slab-thickness-tolerance} (-> (store/ledger db) last :basis)))
    (is (not= 250.0 (:slab-thickness-tolerance-mm (store/batch db "batch-001"))) "fabricated slab-thickness-tolerance never lands in the SSoT")))

(deftest invalid-polish-gloss-level-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t12" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:polish-gloss-level 9999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-polish-gloss-level} (-> (store/ledger db) last :basis)))
    (is (not= 9999.0 (:polish-gloss-level (store/batch db "batch-001"))) "fabricated polish-gloss-level never lands in the SSoT")))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t14" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "saw-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t14")]
